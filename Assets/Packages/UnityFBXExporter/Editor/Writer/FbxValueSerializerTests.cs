...
namespace UnityFBXExporter
{
    public class FbxValueSerializerTests
    {
        ...
        {
            Assert.That(FbxValueSerializer.SerializeCollection(new object[] { 1, 2, 3, 4, 5 }), Is.EqualTo("1,2,3,4,5"));
        }

        [Test]
        public void TestSerializeFloatCollection()
        {
            Assert.That(FbxValueSerializer.SerializeCollection(new object[] { 1.234f, 2f, 3.5f, 4.5f, 5.1f }), Is.EqualTo("1.234,2,3.5,4.5,5.1"));
        }

        [Test]
        public void TestSerializeIntCollectionArrayCheck()
        {
            Assert.That(FbxValueSerializer.Serialize(new object[] { 1, 2 }), Is.EqualTo("1,2"));
        }

        [Test]
        public void TestSerializeFloatCollectionArrayCheck()
        {
            Assert.That(FbxValueSerializer.Serialize(new object[] { 1.1f, 2.2f }), Is.EqualTo("1.1,2.2"));
        }
